...
class GroundTruthRepository(BaseRepository):
    ...
    async def get_canonical_papers(
        ...
        query = """
            SELECT 
                ...
                p.citation_count,
                dcp.relevance_score
            FROM domain_canonical_papers dcp
            JOIN papers p ON dcp.paper_id = p.paper_id
            WHERE dcp.domain = $1
              AND dcp.canonical_tier = $2
              AND dcp.target_audience = $3
            ORDER BY dcp.relevance_score DESC
            LIMIT $4
        """
    ...
    async def create_ground_truth_paper(
        ...
        Updated to match Supabase schema with references_in_corpus field.
        
        Args:
            paper_id: Paper identifier
            num_references: Number of references (reference_count in schema)
            reference_coverage: % of references in corpus
            quality_score: Composite quality score
            is_canonical: Whether paper is canonical for domain
            canonical_tier: Tier if canonical
            
        Returns:
            Record: Created ground truth paper
        """
        logger.info(
        ...
        query = """
            INSERT INTO ground_truth_papers (
                ...
                reference_coverage, quality_score, domain,
        ...
        try:
            result = await self.db.fetchrow(
                query,
                paper_id,
                num_references,  # reference_count in schema
                references_in_corpus,  # NEW field in schema
                reference_coverage,
                quality_score,
                paper_domain,  # domain is required in schema
                is_canonical,
                canonical_tier
            )
